...
console.log(method);
		if( method == 1) {
			...
			it.find("#single").fadeOut(600,function() {
				it.find("#single").css('background-image','url(' + data[m].img + ')');
			}).fadeIn(800);
			
			it.find('.actinfor').fadeOut(650,function() {
				it.find(".actname").text(data[m].name);
				it.find(".actauthor").text(data[m].author);
				it.find(".acttime").text(data[m].time);
				it.find(".illstration").text(data[m].text);
			}).fadeIn(750);
			...
			it.find('.con').fadeOut(200,function() {
